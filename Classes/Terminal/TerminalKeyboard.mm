...
- (TerminalKeyboard*)getKeyboard {
    return keyboard;
}

- (void)deleteBackward
{
    if ([TerminalView editable] && [TerminalView getCurrentCommandLength] > 0)
    {
        [TerminalKeyboard setBackspaceCount :
            [TerminalKeyboard backspaceCount] + 1];
